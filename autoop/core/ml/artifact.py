...
class Artifact(BaseModel):
    ...
    asset_path: str
    version: str
    type: str = Field(..., description="Type of the artifact (e.g., dataset, model)")
    data: Optional[bytes] = None
    metadata: Dict[str, str] = Field(default_factory=dict, description="Metadata for the artifact")
    ...
    def read(self) -> bytes:
        """Simulates reading data for the artifact. In practice, this would interact with storage."""
        if self.data is None:
            raise ValueError("Data not available for this artifact")
        return self.data

    def save(self, data: bytes):
        """Simulates saving data for the artifact. In practice, this would save to storage."""
        self.data = data
