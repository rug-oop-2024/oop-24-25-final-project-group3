...
X = pd.DataFrame({
    "feature_1": np.random.normal(50, 10, n_samples),  # Normally distributed
    "feature_2": np.random.uniform(0, 100, n_samples), # Uniformly distributed
    "feature_3": np.random.binomial(100, 0.5, n_samples),  # Binomial distribution
    "feature_4": np.random.poisson(5, n_samples),  # Poisson distribution
    "feature_5": np.random.exponential(1, n_samples)  # Exponential distribution
})
...
# Using a different number of samples to avoid leaking information from training
n_pred_samples = 200
X_pred = pd.DataFrame({
